...
/// Based on Nouns https://github.com/nounsDAO/nouns-monorepo/blob/master/packages/nouns-contracts/contracts/NounsArt.sol

pragma solidity >=0.8.0;

import {IPlutocatsArt} from "./interfaces/IPlutocatsArt.sol";
import {SSTORE2} from "nouns-monorepo/packages/nouns-contracts/contracts/libs/SSTORE2.sol";
import {IInflator} from "nouns-monorepo/packages/nouns-contracts/contracts/interfaces/IInflator.sol";

contract PlutocatsArt is IPlutocatsArt {
    ...
    function eyesCount() external view returns (uint256) {
        return eyesTrait.storedImagesCount;
    }
    ...
    /**
     * @notice Get a eye image bytes (RLE-encoded).
     */
    ...
    function imageByIndex(IPlutocatsArt.Trait storage trait, uint256 index) internal view returns (bytes memory) {
        (IPlutocatsArt.NounArtStoragePage storage page, uint256 indexInPage) = getPage(trait.storagePages, index);
     ...
     * @return IPlutocatsArt.NounArtStoragePage the page containing the image at index
     * @return uint256 the index of the image in the page
     */
    function getPage(IPlutocatsArt.NounArtStoragePage[] storage pages, uint256 index)
        internal
        view
        returns (IPlutocatsArt.NounArtStoragePage storage, uint256)
    {
        uint256 len = pages.length;
        uint256 pageFirstImageIndex = 0;
        for (uint256 i = 0; i < len; i++) {
            IPlutocatsArt.NounArtStoragePage storage page = pages[i];
    ...
    function decompressAndDecode(IPlutocatsArt.NounArtStoragePage storage page)
        internal
        view
